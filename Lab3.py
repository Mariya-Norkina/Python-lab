...
def task_1_2_process(input_image_path, text, output_image_path, key_file_path):
    print("--- Задание 1.2: LSB кодирование и создание ключа ---")
    ...
    # Подготовка к записи координат и сбору значений пикселей
    bit_idx = 0
    ...
    # 2. Процесс кодирования (проходим по пикселям)
    for y in range(height):
        for x in range(width):
            if bit_idx < total_bits:
                r, g, b = pixels[x, y]
                
                # Для вывода "b" (до изменения)
                if len(original_pixels_preview) < 3:
                    original_pixels_preview.append((r, g, b))
                
                channels = [r, g, b]
                # Записываем в каждый канал по 1 биту (R, G, B)
                for i in range(3):
                    if bit_idx < total_bits:
                ...
                # Для вывода "c" (после изменения)
                if len(changed_pixels_preview) < 3:
                    changed_pixels_preview.append(tuple(channels))

    print(f"b. Исходные значения пикселей (первые 3): {original_pixels_preview}")
    print(f"c. Измененные значения пикселей (первые 3): {changed_pixels_preview}")
    ...
    print(f"Файлы '{output_image_path}' и '{key_file_path}' созданы.\n")
...
if __name__ == "__main__":
    ...
    task_1_2_process('new20.png', 'Name', 'output.png', 'output.txt')
